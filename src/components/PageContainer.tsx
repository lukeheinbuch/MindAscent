...
const PageContainer: React.FC<PageContainerProps> = ({ title, subtitle, actions, children, titleNode, accent = 'none' }) => {
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-8">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="mb-6 md:mb-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
          <div>
            {titleNode ? (
              titleNode
            ) : (
              <h1 className={`text-2xl md:text-3xl font-bold tracking-tight ${accent === 'red-gradient' ? gradientClass : 'text-white'}`}>{title}</h1>
            )}
